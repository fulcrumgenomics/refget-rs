...
async fn main() -> anyhow::Result<()> {
    ...
    let (sequence_store, seqcol_store) = match args.mode {
        StoreMode::Memory => load_memory_mode(&fasta_files, &config.circular_sequences)?,
        StoreMode::Disk => load_disk_mode(&fasta_files, &config.circular_sequences)?,
    };
